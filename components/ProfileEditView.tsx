import {Link, Text, Stack, Input, Textarea, FormControl, FormLabel, FormErrorMessage, Avatar, useDisclosure, Modal, ModalOverlay, ModalContent, Box, ModalCloseButton, ModalBody} from "@chakra-ui/react";
...
export default function ProfileEditView(props: ProfileEditViewProps) {
    ...
    const onDrop = (acceptedFiles: any) => {
        ...
        reader.onload = () => props.setProfileImageFile(reader.result);
        ...
        <Stack spacing={3}>
            <div>
                <div style={{display: "flex", flexDirection: "row"}}>
                    <Avatar style={{width: "125px", height: "125px", borderRadius: "100%"}} name={props.firstName + " " + props.lastName === " " ? "": props.firstName + " " + props.lastName} draggable="false" src={props.profileImageFile === "" ? props.profileImage: props.profileImageFile}/>
                    <div style={{paddingLeft: "15px", width: "100%"}}>
                        <Stack spacing={4}>
                            <div>
                                <FormControl isInvalid={props.isMajorError}>
                                    <FormLabel style={{marginBottom: "0px"}}>Major <Text as="span" style={{color: "#E53E3E"}}>*</Text></FormLabel>
                                    <Input onBlur={e => {if(e.target.value.length === 0) props.setIsMajorError(true); else props.setIsMajorError(false);}} value={props.major} onChange={e => props.setMajor(e.target.value)} style={{borderRadius: "10px"}} size="xs" placeholder={"Major"} />
                                    {props.isMajorError ? <FormErrorMessage style={{margin: "0px", padding: "0px"}}></FormErrorMessage>: null}
                                </FormControl>
                            </div>
                            <div>
                                <FormControl isInvalid={props.isYearError}>
                                    <FormLabel style={{marginBottom: "0px"}}>Graduating Year <Text as="span" style={{color: "#E53E3E"}}>*</Text></FormLabel>
                                    <Input onBlur={e => {if(e.target.value.length === 0) props.setIsYearError(true); else props.setIsYearError(false);}} value={props.gradYear} onChange={e => props.setYear(e.target.value)} style={{borderRadius: "10px"}} size="xs" placeholder={"Graduation Year"} />
                ...
                <Link onClick={onOpen} style={{marginLeft: "7px"}} variant="photo">Add new photo</Link>
                <Modal isOpen={isOpen} onClose={onClose} isCentered>
                    <ModalOverlay />
                    <ModalContent maxW="1200px">
                        <ModalCloseButton />
                        <ModalBody>
                            <Box style={{marginTop: "40px", marginBottom: "20px", marginLeft: "20px", marginRight: "20px"}} w="95%" {...getRootProps()} display="grid" placeItems={"center"} minW="20rem" minH="15rem" border="1px dashed black" cursor="pointer">
                                <input {...getInputProps()} />
                                <Text>{isDragActive ? "Release here to upload your image.": "Drop an image here, or click to select a file."}</Text>
                            </Box>
                        </ModalBody>
                    </ModalContent>
                </Modal>
            ...
            <div>
                <FormControl isInvalid={props.isFirstNameError}>
                    <FormLabel style={{marginBottom: "0px"}}>First Name <Text as="span" style={{color: "#E53E3E"}}>*</Text></FormLabel>
                    <Input onBlur={e => {if(e.target.value.length === 0) props.setIsFirstNameError(true); else props.setIsFirstNameError(false);}} value={props.firstName} style={{borderRadius: "10px"}} onChange={e => props.setFirstName(e.target.value)} size="xs" placeholder={"First Name"} />
                    {props.isFirstNameError ? <FormErrorMessage style={{margin: "0px", padding: "0px"}}></FormErrorMessage>: null}
                </FormControl>
            </div>
            <div>
                <FormControl isInvalid={props.isLastNameError}>
                    <FormLabel style={{marginBottom: "0px"}}>Last Name <Text as="span" style={{color: "#E53E3E"}}>*</Text></FormLabel>
                    <Input onBlur={e => {if(e.target.value.length === 0) props.setIsLastNameError(true); else props.setIsLastNameError(false);}} value={props.lastName} style={{borderRadius: "10px"}} onChange={e => props.setLastName(e.target.value)} size="xs" placeholder={"Last Name"} />
                    {props.isLastNameError ? <FormErrorMessage style={{margin: "0px", padding: "0px"}}></FormErrorMessage>: null}
                </FormControl>
            </div>
            <div>
                <FormControl isInvalid={props.isEmailError}>
                    <FormLabel style={{marginBottom: "0px"}}>Email <Text as="span" style={{color: "#E53E3E"}}>*</Text></FormLabel>
                    <Input onBlur={e => {if(!emailRe.test(e.target.value)) props.setIsEmailError(true); else props.setIsEmailError(false);}} value={props.email} style={{borderRadius: "10px"}} onChange={e => props.setEmail(e.target.value)} size="xs" placeholder={"Email"} />
                    {props.isEmailError ? <FormErrorMessage style={{margin: "0px", padding: "0px"}}></FormErrorMessage>: null}
                </FormControl>
            </div>
            <div>
                <FormControl>
                    <FormLabel style={{marginBottom: "0px"}}>Bio</FormLabel>
                    <Textarea style={{color: "#1c1c1c", borderRadius: "10px"}} resize="none" onChange={e => props.setBio(e.target.value)} value={props.userBio} size="xs" placeholder={"Enter a short biography, introducing yourself and highlighting your academic and nonacademic interests"} />
                </FormControl>
            </div>
            <div>
                <FormControl>
                    <FormLabel style={{marginBottom: "0px"}}>Pronouns</FormLabel>
                    <Input onChange={e => props.setPronouns(e.target.value)} style={{borderRadius: "10px"}} size="xs" value={props.pronouns} placeholder={"Pronouns"} />
                </FormControl>
            </div>
            ...
            <div style={{display: "flex", flexDirection: "row"}}>
                ...
                <Input style={{borderRadius: "10px"}} onChange={e => props.setFBURL(e.target.value)} size="xs" value={props.fbURL} placeholder={"Enter your facebook profile link"} />
            </div>
            <div style={{display: "flex", flexDirection: "row"}}>
                <div style={{paddingRight: "15px"}}>
                ...
                <Input style={{borderRadius: "10px"}} onChange={e => props.setIGURL(e.target.value)} size="xs" value={props.igURL} placeholder={"Enter your instagram profile link"} />
            </div>
            <div style={{display: "flex", flexDirection: "row"}}>
                <div style={{paddingRight: "15px"}}>
                    <svg fill="#414141" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 30 30" width="30px" height="30px">
                ...
                <Input style={{borderRadius: "10px"}} onChange={e => props.setSCURL(e.target.value)} size="xs" value={props.scURL} placeholder={"Enter your snapchat profile link"} />
            </div>
        </Stack>
    );
}