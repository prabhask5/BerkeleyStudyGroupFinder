...
import { Button, ButtonGroup, CloseButton, Divider, Heading, IconButton, Stack, Text, useToast} from "@chakra-ui/react";
...
export default function MyProfile() {
    ...
    const [profileImage, setProfileImage] = useState(placeHolderData.myData.profileImage);
    const [major, setMajor] = useState(placeHolderData.myData.major);
    const [year, setYear] = useState(placeHolderData.myData.gradYear);
    const [firstName, setFirstName] = useState(placeHolderData.myData.firstName);
    const [lastName, setLastName] = useState(placeHolderData.myData.lastName);
    const [bio, setBio] = useState(placeHolderData.myData.userBio);
    const [pronouns, setPronouns] = useState(placeHolderData.myData.pronouns);
    const [fbURL, setFBURL] = useState(placeHolderData.myData.fbURL);
    const [scURL, setSCURL] = useState(placeHolderData.myData.igURL);
    const [igURL, setIGURL] = useState(placeHolderData.myData.scURL);
    const [email, setEmail] = useState(placeHolderData.myData.email);
    ...
    const handleChangeProfile = (major: string, year: string, firstName: string, lastName: string, bio: string, pronouns: string, fbURL: string, scURL: string, igURL: string) => {
    ...
    const profileView = editState === 0 ? <div style={{marginTop: "6%", marginRight: "3%"}}>
        <Stack spacing={7}>
            ...
            <Stack spacing={7}>
                <ProfileReadView profileImage={profileImage} firstName={firstName} lastName={lastName} major={major} gradYear={year} userBio={bio} pronouns={pronouns} fbURL={fbURL} igURL={fbURL} scURL={scURL} />
                <div style={{display: "flex", flexDirection: "row"}}>
                    <a style={{paddingRight: "15px"}} target="_blank" rel="noreferrer" href={fbURL}>
                    ...
                    <a style={{paddingRight: "15px"}} target="_blank" rel="noreferrer" href={igURL}>
                    ...
                    <a target="_blank" rel="noreferrer" href={scURL}>
                        <svg className="icon" fill="#414141" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 30 30" width="30px" height="30px">
                        ...
                        {userCourseData.map(function(d) {
                                return (
                                    <div key={d.id}>
                                        ...
                                        <div style={{display: "grid", gridTemplateColumns: "93.5% 5%", }}>
                                            <CourseListItemLower classNumber={d.classNumber} classDis={d.classDis} classLab={d.classLab} />
